...
import {
    ...
    saveEntrust,
    approveEntrustPlan,
    returnEntrustPlan, removeEntrust
} from '../api'
...
class HomeForm extends Component {
    ...
    handleZatwierdzPlan() {
        approveEntrustPlan(this.state.selectedPlan).then((response) => {
            ...
            if(error.response.data !== undefined && error.response.status === 400) {
                error.response.data.map(obj => {
                    toast.notify(obj)
                })
            }else
            {
                toast.notify("Zatwierdzenie niemożliwe")
            }
        })
    }
        ...
        if (save) {
            this.toUpdate.map((obj) => {
                ...
                if (toDelete) {
                    ...
                    removeEntrust(pow).then((response) =>{
                        if (response.status === 200) {
                            ...
                            toast.notify("Powierzenie " + obj.prow.imie + " " + obj.prow.nazwisko + " na " + this.state.selectedKurs.nazwa + " usunięte!");
                        }
                    }).catch(error => {
                        toast.notify("Usunięcie powierzenia " + obj.prow.imie + " " + obj.prow.nazwisko + " na " + this.state.selectedKurs.nazwa + " nie powiodło sie!");
                    })
                } else {
                    let plan = {
                        "powierzenia": [
    ...
    loadData() {
        getSubjectData({
            ...
            this.setState({
                kurses: modulDto.reduce((result, item) => result.concat(
                    item.kursy.map(kurs => ({
                        id: kurs.id,
                        nazwa: kurs.nazwa,
                ...
                <Row>
                    <Col>
                        <Form.Group className="d-flex justify-content-between">
                            ...
                            <Button variant="primary" onClick={this.logoff}>Wyloguj</Button>
                        </Form.Group>
                    </Col>
                </Row>
                <Row>
                    <Col md={3}>
                        <Form.Group>
                            <div>
                                <Select options={this.state.entrustPlans.map(x => x.nazwa)}
                                        onChange={this.handleChangeEntrustPlan}/>
                            </div>
                        </Form.Group>
                    </Col>
                    <Col md={2}>
                    ...
                    <Col md={5}>
                        <Form.Control as="input" ref={this.state.name}
                        ...
                        {this.state.mergedKurses.map((kurs, index) => (
                            <tr key={index}>
                                ...
                                <td>
                                    ...
                                    <td><Button className="round-button"
                                                onClick={() => this.handleShow(kurs)}>+</Button></td>}
                                </td>
                            </tr>
                        ))}
                        </tbody>
                                ...
                                <Table striped bordered hover>
                                    ...
                                    {this.state.updatedProwadzacy.map((prow, index) => (
                                        <tr>
                                            ...
                                            <td><FormControl
                                                defaultValue={this.state.selectedPlan.powierzenia.filter(x => x.prowadzacy.id === prow.id && x.kurs.id === this.state.selectedKurs.id).map(x => x.liczbaGodzin).reduce((x, y) => x + y, 0)}
                                                type="text" className="mr-sm-2"
                                                onChange={(eve) => this.handleNumberOfHours(eve.target.value, prow)}/>
                                            </td>
                                        </tr>
                                    ))}
                                    </tbody>
